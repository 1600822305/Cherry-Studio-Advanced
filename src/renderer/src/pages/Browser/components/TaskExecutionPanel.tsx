...
import { Button, Card, Progress, Space, Steps, Typography, Collapse } from 'antd'
        ...
        >
            ...
            {task.subtasks && task.subtasks.length > 0 ? (
                <>
                    ...
                    <Paragraph>
                        ...
                        <Text>
                            复杂任务已分解为 {task.subtasks.length} 个子任务
                        </Text>
                    </Paragraph>
                            ...
                            <Panel
                                header={
                                    <Space>
                                        ...
                                        <Text strong>子任务 {index + 1}: {subtask.description}</Text>
                                        <Text type={subtask.completed ? "success" : subtask.error ? "danger" : "warning"}>
                                            {subtask.completed ? "已完成" : subtask.error ? "出错" : `进行中 (${Math.round((subtask.currentStep / subtask.steps.length) * 100)}%)`}
                                        </Text>
                                    </Space>
                                }
                                key={index.toString()}
                            >
                                ...
                                {subtask.completed && subtask.result && (
                                    <Card type="inner" title="子任务结果" size="small" style={{ marginTop: 16 }}>
                                        <Paragraph>
                                            {subtask.result.length > 100
                                                ? subtask.result.substring(0, 100) + '...'
                                                : subtask.result}
                                        </Paragraph>
                                    </Card>
                                )}
            ...
            {task.completed && !task.error && task.result && (
                <Card
                    type="inner"
                    title="执行结果"
                    extra={onShowResult && <Button type="link" onClick={() => onShowResult(task.result || '')}>查看详情</Button>}
                >
                    <Paragraph>
                        任务已成功完成。
